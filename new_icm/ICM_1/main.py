...
if __name__ == '__main__':
    ...
    optimizer1 = SharedAdam(shared_model1.parameters(), lr= args.lr)
    optimizer2 = SharedAdam(shared_model2.parameters(), lr= args.lr)

    optimizer1.share_memory()
    ...
    for rank in range(0, num_procs):
        if rank < sample_val:                           # select random action
            p = mp.Process(target=train, args=(rank, args, shared_model1, shared_model2, counter, lock, optimizer1, optimizer2))
        else:                                           # select best action
            p = mp.Process(target=train, args=(rank, args, shared_model1, shared_model2 , counter, lock, optimizer1, optimizer2, False))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()