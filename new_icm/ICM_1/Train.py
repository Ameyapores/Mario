...
def train(rank, args, shared_model1, shared_model2, counter, lock, optimizer1=None, optimizer2= None, select_sample=True):
    ...
    if optimizer1 is None:
        optimizer1 = optim.Adam(shared_model1.parameters(), lr=args.lr)
        
    if optimizer2 is None:
        optimizer2 = optim.Adam(shared_model2.parameters(), lr=args.lr)
    ...
    for num_iter in count():
        if rank == 0:
            if num_iter % args.save_interval == 0 and num_iter > 0:
                ...
                torch.save(shared_model1.state_dict(), args.save_path1)            
        ...
        for step in range(args.num_steps):
            ...
            s_t1 = state
            birth = Variable(s_t.unsqueeze(0)).type(FloatTensor), Variable(s_t1.unsqueeze(0)).type(FloatTensor), a_t
            #print (birth)
            vec_st1, inverse, forward = model2(birth)
            ...
            for i in reversed(range(len(rewards))):
                ...
                ensure_shared_grads(model1, shared_model1)
