...
def get_args():
    ...
    parser.add_argument('--env', default='SuperMarioBros-v0', type=str, help='gym environment')
...
def cost_func(args, values, logps, actions, rewards, inverses, forwards, vec_st1s):
    ...
    #cross_entropy= -(torch.tensor(actions)
    #entropy_loss = -(-logps * torch.exp(logps)).sum() # encourage lower entropy
    #cross_entropy=-(Variable(actions).view(-1, 1) * torch.log(Variable(inverses).view(-1, 1) +1e-15))
    #inverse_loss= cross_entropy
    forward_err= torch.tensor(forwards) - torch.tensor(vec_st1s)
    forward_loss= 0.5* (forward_err.pow(2)).sum()
    print (forward_loss, value_loss, policy_loss)
    return ((args.beta * forward_loss )+ args.lmbda * (policy_loss + 0.5 * value_loss))
...
def train(shared_model, shared_optimizer, rank, args, info):
    ...
    while info['frames'][0] <= 4e7 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):
            ...
            values.append(value) ; logps.append(logp) ; actions.append(action) ; rewards.append(reward) ; vec_st1s.append(vec_st1) ; inverses.append(inverse); forwards.append(forward)
